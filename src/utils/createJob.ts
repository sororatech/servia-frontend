...
export async function createJob(payload: CreateJobPayload): Promise<CreateJobResult> {
  ...
  const response = await fetch(getApiUrl('/jobs/jobs/'), {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    ...
    console.error('[createJob] backend error', response.status, text.slice(0, 500));
    const message = extractFirstError(data) ?? `Failed to create job (${response.status})`;
