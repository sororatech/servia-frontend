                ...
                <tr key={c.id} className="hover:bg-black/[0.02]">
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <p className="font-medium text-[#171717]">{c.first_name} {c.last_name}</p>
                    </div>
                  </td>
                  <td className="px-4 py-4 text-sm text-[#635b55]">{c.email}</td>
                  <td className="px-4 py-4 text-sm text-[#635b55]">{c.applied_date ? new Date(c.applied_date).toLocaleDateString() : 'N/A'}</td>
                </tr>
              ))
            )}
          </tbody>
