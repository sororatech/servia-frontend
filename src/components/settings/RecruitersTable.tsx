...
export default function RecruitersTable({
  ...
  const handleToggleStatus = async (recruiter: Recruiter) => {
    // ✅ FIXED: Use nested user name for confirmation
          ...
          <tbody className="divide-y divide-black/5">
            {filteredRecruiters.length === 0 ? (
              ...
              filteredRecruiters.map((r) => {
                // ✅ FIXED: Safe access for avatar initials
                const firstName = r.user?.first_name || '';
                const lastName = r.user?.last_name || '';
                  ...
                  <tr key={r.id} className="hover:bg-black/[0.02]">
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          {/* ✅ FIXED: Display nested user name and email */}
...
<button onClick={() => handleToggleStatus(r)} className={`rounded-lg p-2 ${r.is_active ? 'text-red-600 hover:bg-red-50' : 'text-green-600 hover:bg-green-50'}`}>                          {r.is_active ? '🚫' : '✅'}
                        </button>
