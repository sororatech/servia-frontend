...
export default function SystemHealthTab() {
  ...
  if (loading) {
    return (
      <div className="flex min-h-[300px] items-center justify-center rounded-2xl border border-[var(--color-warm-border)] bg-white dark:bg-[var(--color-warm-surface)] p-12 shadow-sm">
        <div className="animate-pulse space-y-4 text-center">
          <div className="h-4 w-48 rounded bg-[var(--color-warm-border)] mx-auto" />
          <div className="h-3 w-32 rounded bg-[var(--color-warm-border-light)] mx-auto" />
        </div>
      </div>
    );
  }
  ...
  const statusIcon = (status: 'ok' | 'error') => status === 'ok' ? '✓' : '✗';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="rounded-2xl border border-[var(--color-warm-border)] bg-white dark:bg-[var(--color-warm-surface)] p-6 shadow-sm">
          ...
          <p className={`mt-2 text-2xl font-bold ${
            data.status === 'healthy' ? 'text-[var(--color-status-active-text)]' :
            data.status === 'degraded' ? 'text-[var(--color-status-warning-text)]' : 'text-[var(--color-status-error-text)]'
          }`}>
      ...
      {/* Service health */}
      <div className="rounded-2xl border border-[var(--color-warm-border)] bg-white dark:bg-[var(--color-warm-surface)] p-6 shadow-sm">
        <h3 className="text-lg font-bold text-[var(--color-foreground)] mb-4">Service Health</h3>
        <div className="grid grid-cols-2 gap-4">
          {Object.entries(data.services).map(([service, status]) => (
            <div key={service} className="flex items-center justify-between rounded-xl border border-[var(--color-warm-border)] bg-[var(--color-warm-bg)] dark:bg-[var(--color-warm-bg-deep)] p-3">
              <span className="text-sm font-medium capitalize text-[var(--color-text-dark)] dark:text-[var(--color-foreground)]">{service}</span>
      ...
      {/* API usage */}
      <div className="rounded-2xl border border-[var(--color-warm-border)] bg-white dark:bg-[var(--color-warm-surface)] p-6 shadow-sm">
        <h3 className="text-lg font-bold text-[var(--color-foreground)]">API Usage</h3>
        ...
        <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-[var(--color-warm-border)]">
          <div
      ...
      <div className="rounded-2xl border border-[var(--color-warm-border)] bg-white dark:bg-[var(--color-warm-surface)] p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
          <h3 className="text-lg font-bold text-[var(--color-foreground)]">Error Log</h3>
          {data.sentry.configured && (
            <a href={data.sentry.project_url} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-[var(--color-primary)] hover:text-[var(--color-primary-hover)]">
              View in Sentry →
            </a>
          )}
        </div>
          ...
          <div className="space-y-3 max-h-80 overflow-y-auto">
            {data.recent_errors.map((err) => (
              <a
                ...
                className="block rounded-xl border border-[var(--color-warm-border)] p-4 transition hover:bg-[var(--color-warm-bg)] dark:hover:bg-[var(--color-warm-bg-deep)]"
              >
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    <p className="font-semibold text-[var(--color-foreground)]">{err.title}</p>
                    {err.culprit && <p className="mt-1 font-mono text-xs text-[var(--color-text-muted)]">{err.culprit}</p>}
                    <p className="mt-2 text-xs text-[var(--color-text-subtle)]">
                      {new Date(err.first_seen).toLocaleString()} • {err.count} occurrence{err.count !== 1 ? 's' : ''}
                    </p>
                  </div>
                  <span className={`rounded-full px-2 py-1 text-xs font-medium ${
                    err.level === 'error' ? 'bg-[var(--color-status-error-bg)] text-[var(--color-status-error-text)]' :
                    err.level === 'warning' ? 'bg-[var(--color-status-warning-bg)] text-[var(--color-status-warning-text)]' :
                    'bg-[var(--color-warm-surface)] dark:bg-[var(--color-warm-bg-deep)] text-[var(--color-text-subtle)]'
                  }`}>
                    {err.level}
                  </span>
                </div>
