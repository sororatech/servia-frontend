...
export default async function SystemSettingsPage({
  ...
  try {
    ...
    const token = (headers as Record<string, string>)['Authorization']?.replace('Token ', '');
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(38,185,200,0.12),_transparent_22%),linear-gradient(180deg,#fbfaf8_0%,#f3ece7_100%)] px-4 py-8 sm:px-6 lg:px-10 overflow-y-auto">
      <div className="mx-auto max-w-[1400px]">
        ...
        <div className="mb-8 flex gap-2 border-b border-black/10">
          <TabButton active={tab === 'users'} href="/recruiter/dashboard/settings?tab=users" icon="users">User Management</TabButton>
          <TabButton active={tab === 'health'} href="/recruiter/dashboard/settings?tab=health" icon="health">API & Health</TabButton>
        </div>
