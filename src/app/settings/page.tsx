...
function TabButton({ active, href, icon, children }: { active: boolean; href: string; icon: string; children: React.ReactNode }) {
  ...
  return (
    <a href={href} className={`flex items-center rounded-lg px-6 py-3 text-sm font-medium transition-all ${active ? 'bg-[#26b9c8] text-white shadow-lg shadow-[#26b9c8]/25' : 'bg-white/60 text-[#635b55] hover:bg-white/90'}`}>
      {icons[icon]}
      {children}
    </a>
  );
