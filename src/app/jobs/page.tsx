      ...
      <header className="bg-white border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-8 py-5 flex items-center justify-between">
          ...
          <nav className="flex gap-8">
            <button className="text-teal-600 font-semibold text-base border-b-2 border-teal-600 pb-1">Browse Jobs</button>
            <button onClick={goToDashboard} className="text-gray-400 hover:text-gray-600 font-medium text-base">My Applications</button>
          </nav>
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={goToProfile} className="w-10 h-10 bg-gray-200 rounded-full flex items-center justify-center hover:bg-gray-300 transition-colors" title="Profile">
          ...
          <div className="sticky top-8">
            ...
            <div className="mb-8">
              ...
              <div className="flex flex-wrap gap-2">
                {['Full-time', 'Part-time', 'Contract'].map(type => (
                  <button key={type} onClick={() => toggleJobType(type.toLowerCase())}
                    className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all cursor-pointer ${filters.jobType.includes(type.toLowerCase()) ? 'bg-teal-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                    {type}
                  </button>
                ))}
              </div>
            ...
            <div className="mb-8">
              ...
              <div className="space-y-1">
                {Object.entries(departmentGroups).map(([category, depts]) => {
                  ...
                  return (
                    <div key={category}>
                      <div onClick={() => toggleCategory(category)} className="flex items-center justify-between py-2 cursor-pointer hover:bg-gray-50 rounded px-2 transition-colors">
                        <div className="flex items-center gap-2 flex-1">
                          <span className="text-sm font-semibold text-gray-800">{category}</span>
                          {selectedCount > 0 && <span className="text-xs text-teal-600 font-medium">({selectedCount})</span>}
                        </div>
                      ...
                      {isExpanded && (
                        <div className="pl-4 space-y-0.5 pb-2">
                          {depts.map(dept => (
                            <div key={dept.value} onClick={() => toggleDepartment(dept.value)}
                              className={`flex items-center gap-2 cursor-pointer py-1.5 px-2 rounded text-sm transition-colors ${filters.department.includes(dept.value) ? 'bg-teal-50 text-teal-700 font-medium' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}>
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {filters.department.map(val => {
                    ...
                    return (
                      <span key={val} className="inline-flex items-center gap-1 px-2 py-1 bg-teal-50 text-teal-700 text-xs rounded-md">
                        {label}
                        <button onClick={() => toggleDepartment(val)} className="hover:text-teal-900">
            ...
            {(filters.jobType.length > 0 || filters.department.length > 0 || searchQuery) && (
              <div className="pt-6 border-t border-gray-100">
                <button onClick={clearFilters} className="text-sm text-teal-600 hover:text-teal-700 font-medium">Clear all filters</button>
              </div>
            )}
          </div>
        </aside>
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              {(searchQuery || filters.jobType.length > 0 || filters.department.length > 0) && (
                <button onClick={clearFilters} className="text-teal-600 hover:text-teal-700 font-medium text-sm">Clear filters to see all jobs</button>
              )}
            </div>
          ) : (
              ...
              {jobs.map(job => {
                ...
                return (
                  <div key={job.id} className="bg-white rounded-2xl p-6 border border-gray-100 hover:shadow-xl hover:border-gray-200 transition-all cursor-pointer" onClick={() => goToJob(job.id)}>
                    ...
                    <p className="text-sm text-teal-600 font-medium mb-4">{job.location}</p>
                    ...
                    <button className={`w-full py-3 rounded-xl font-semibold text-sm transition-colors ${deadlineText?.text === 'Closed' ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-teal-500 text-white hover:bg-teal-600'}`} disabled={deadlineText?.text === 'Closed'}>
