  ...
  const handleApply = useCallback(async () => {
    ...
    try {
      ...
      if (!result.already_exists) {
        ...
        const department = encodeURIComponent(job?.department || '');


        router.push(`/candidate/dashboard/cv?application=${applicationId}&jobTitle=${jobTitle}&department=${department}`);
        return;
      }
      ...
      if (error.message.startsWith('VALIDATION_ERROR:')) {
        ...
        try {
          ...
          if (parsed.already_exists && parsed.id) {
            setErrorMessage('You have already applied to this job.');
            return;
          }
